...
// BREUK NAAR PERCENTAGE FUNCTIE
// ============================================
function generateFractionPercentageQuestion() {
  // Genereer willekeurige teller en noemer (beide tussen 1 en 100)
  let numerator = floor(random(1, 100));    // 1 tot 100
  let denominator = floor(random(2, 100));  // 1 tot 100
  
  // Bereken het percentage (afgerond op hele getallen)
  let percentage = round((numerator / denominator) * 100);
  
  // Maak de vraag string (zonder percentage teken in de breuk zelf)
  let questionText = numerator + '/' + denominator;
  
  return {
    question: questionText,
    answer: percentage,
    explanation: questionText + ' = ' + percentage + '%'
  };
}
...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        let q = generateFractionPercentageQuestion();
        questions.push({
            text: q.question + ' = ?%',
            answer: q.answer
        });
...
function drawBlock(block) {
  ...
  // Hover: pas alleen de schaduw/visuele effecten toe, geen translate/scale
  if (block.isQuestion && block.hoverProgress > 0) {
    let lift = -6 * block.hoverProgress;
    drawY += lift;  // ← Verschuif gewoon drawY, geen scale
  ...
  stroke(50, 100, 200);
  strokeWeight(3);
  rect(drawX + 5, drawY + 5, CELL_SIZE - 10, CELL_SIZE - 10, 5);
  ...
  if (block.text.includes('/')) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (part.includes('/')) {
        ...
        textSize(16);
        text(num, startX + 15, centerY - 10);
        stroke(0);
        strokeWeight(2);
        line(startX + 5, centerY, startX + 25, centerY);
        noStroke();
        text(den, startX + 15, centerY + 15);
        startX += 35;
      } else if (part === '=') {
        textSize(16);
        text('=', startX, centerY);
    ...
    textSize(16);
    text(block.text, drawX + CELL_SIZE / 2, drawY + CELL_SIZE / 2);
  }
